...
class ChatInterface(QMainWindow, Ui_MainWindow):
    ...
    def about(self):
        QMessageBox.about(
            self,
            "About Qtcord",
            f"<p>Qtcord {get_version()}</p>"
            "<p>(c) mak448a 2023-present</p>"
        ...
        for friend in self.friends:
            ...
            while True:
                ...
                try:
                    ...
                    button.clicked.connect((lambda uid: lambda: self.switch_dms(uid))(uid))
                    break
    ...
    def switch_dms(self, user_id: str) -> None:
            while True:
                ...
                try:
                    ...
                    raise Exception (
                            f"Warning: Could not get channel for user {user.get_user_name()} (ID: {user.id}): {e}"
                    )
                    break
